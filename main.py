...
#        self.fc = nn.Linear(512 * block.expansion, 512)
        self.fc = nn.Linear(512 * block.expansion, 2048)  # 変更点: 512を2048に
...
#         )
        self.fc = nn.Sequential(
        nn.Linear(2048 + 768, 512),  # 変更点: 1024を2048 + 768に変更
        nn.ReLU(inplace=True),
        nn.Linear(512, n_answer)
        )
    ...
    def forward(self, image, input_ids, attention_mask):
        ...
        question_feature = text_outputs.last_hidden_state[:, 0, :]
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=64, shuffle=True)#バッチサイズを64に変更
